...
### Apartir de aqui todo es empleador ###
def search_user_db_employer(user_id: str):
    try:
        user = db_client.employers.find_one({"id": user_id})
        if user:
            return UserDB(**user_schema(user))
        else:
            return None  # Si no se encuentra el usuario, devolver None
...
def search_user_employer(user_id: str):
    try:
        user = db_client.employers.find_one({"id": user_id})
        if user:
            return User(**user_schema(user))
...
# Autenticacion de usuario encriptado
async def auth_user_employer(token: str = Depends(oauth2)):
    exception = HTTPException(
        status_code=status.HTTP_401_UNAUTHORIZED,
        detail="Credenciales de autenticación inválidas",
        headers={"WWW-Authenticate": "Bearer"},
    ...
    # Utiliza la función search_user_graduate para buscar el usuario en la base de datos
    user = search_user_employer(user_id)
...
# si el usuario está inactivo (disabled), enviar mensaje
async def current_user_employer(user: User = Depends(auth_user_employer)):
    # Utiliza la función search_user_graduate para obtener el usuario actualizado de la base de datos
    user = search_user_employer(user.id)
